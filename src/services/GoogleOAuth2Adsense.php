 ...
 */
class GoogleOAuth2Adsense extends Service
{
    ...
    protected $name = 'google_adsense';
    protected $title = 'Google';
    protected $type = 'OAuth2';
    protected $jsArguments = ['popup' => ['width' => 500, 'height' => 450]];

    protected $scopes = ['https://www.googleapis.com/auth/adsense'];
    ...
    {
        //echo 111;
        //print_r($this->makeSignedRequest('http://only-top.ru'));
        ...
        $info = $this->makeSignedRequest('https://www.googleapis.com/adsense/v1.4/accounts');

        $account = $info['items'][0]['id'];
        //print_r($account);
        //print_r($info);

        //формируем строку для данного id
        $url_google = "https://www.googleapis.com/adsense/v1.4/accounts/".$account."/reports?";
        ...
        for($i=0; $i < 12; $i++){
            ...
            $info = $this->makeSignedRequest($url_google.
                'endDate='.$end_day.
                '&startDate='.$start_day.
                '&currency=INR&dimension=DOMAIN_NAME&metric=EARNINGS&fields=rows'
            );
            $money = 0;
            foreach($info['rows'] as $item){
                if(in_array($item[0], $mask_url)){
                    $money = $item[1] + $money;
                    if($money > $max_money){
                        $max_money = $money;
                    }
        ...
        $this->attributes['max_money'] = $max_money;
